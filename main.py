...
from agents.dqn_agent import DQNAgent
from agents.double_dqn_agent import DoubleDQNAgent  # Import the Double DQN agent
from utils.train_logger import TrainLogger
...
def train(
    ...
    # Make sure we actually return the results dictionary
    return {
        "episodes": i_episode,
        "final_avg_score": metrics['avg_score'],
        "final_success_rate": metrics['success_rate'],
...
def main() -> None:
    ...
    # Check if results is None before trying to access it
    if results is None:
        print("Training did not complete successfully. No results returned.")
        env.close()
        return
